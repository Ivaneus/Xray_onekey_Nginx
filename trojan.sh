...
function system_check() {
  ...
  if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]]; then
    ...
    INS="yum install -y"
    wget -N -P /etc/yum.repos.d/ https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/basic/nginx.repo
  elif [[ "${ID}" == "ol" ]]; then
...
function dependency_install() {
  ...
  if ! command -v jq; then
    wget -P /usr/bin https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/binary/jq && chmod +x /usr/bin/jq
    judge "安装 jq"
  fi
...
function basic_optimization() {
  ...
  echo "DefaultLimitSIGPENDING=102400" >> /etc/systemd/system.conf
  echo "DefaultLimitSTACK=102400" >> /etc/systemd/system.conf
  echo "DefaultLimitMEMLOCK=102400" >> /etc/systemd/system.conf
  echo "DefaultLimitMSGQUEUE=102400" >> /etc/systemd/system.conf  
...
function update_sh() {
  ol_version=$(curl -L -s https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [[ "$shell_version" != "$(echo -e "$shell_version\n$ol_version" | sort -rV | head -1)" ]]; then
    print_ok "存在新版本，是否更新 [Y/N]?"
    read -r update_confirm
    case $update_confirm in
    [yY][eE][sS] | [yY])
      wget -N --no-check-certificate https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/install.sh
      print_ok "更新完成"
...
function configure_nginx_temp(){
  ...
  wget -O ${domain}.conf https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/config/web_temp.conf
...
function configure_nginx() {
  ...
  if [[ $compatible_nginx_conf == "yes" ]]; then
    wget -O ${domain}.conf https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/config/web_compatible.conf
  elif [[ $compatible_nginx_conf == "no" ]]; then
    wget -O ${domain}.conf https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/config/web.conf
  fi
  sed -i "s/xxx/${domain}/g" ${nginx_conf}
  modify_port
  modify_nginx_other
...
function configure_xray_ws() {
  cd /usr/local/etc/xray && rm -f config.json && wget -O config.json https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/${github_branch}/config/xray_tls_ws.json
  modify_UUID
  modify_ws
  modify_inbound_port
}
...
function configure_web() {
  ...
  case $webpage in
  [yY][eE][sS] | [yY])
    wget -O web.tar.gz https://raw.githubusercontent.com/Ivaneus/Xray_onekey_Nginx/main/basic/web.tar.gz
